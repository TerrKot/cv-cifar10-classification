#!/usr/bin/env python3
"""
Обучение Improved CNN на 25 эпохах
"""
...
from models.improve_cnn import ImprovedCNN, get_augmented_data_loaders, count_parameters

def train_improved_25_epochs():
    """Обучение Improved CNN на 25 эпохах"""
    
    print("🚀 ОБУЧЕНИЕ IMPROVED CNN НА 25 ЭПОХАХ")
    print("=" * 60)
    ...
    # Создаем модель
    model = ImprovedCNN(num_classes=10, dropout_rate=0.3).to(device)  # Уменьшаем dropout
    ...
    # Результаты
    print(f"\n📈 РЕЗУЛЬТАТЫ IMPROVED CNN (25 ЭПОХ):")
    ...
    # Сохранение модели
    model_path = 'models/improved_25epochs_model.pth'
    torch.save({
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title('Improved CNN (25 epochs): Loss по эпохам')
    ax1.legend()
    ...
    ax2.set_title('Improved CNN (25 epochs): Accuracy по эпохам')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('results/25_epochs/improved_25epochs_results.png', dpi=150, bbox_inches='tight')
    plt.show()
    
    print(f"✅ Графики сохранены в results/25_epochs/improved_25epochs_results.png")
    
    # Сохранение отчета
    print(f"\n📝 Создание отчета...")
    
    filename = 'results/25_epochs/improved_25epochs_report.txt'
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("ОТЧЕТ ОБУЧЕНИЯ IMPROVED CNN НА 25 ЭПОХАХ\n")
        f.write("=" * 50 + "\n\n")
        
        f.write(f"Модель: Improved CNN (25 epochs)\n")
    ...
    print(f"  - results/25_epochs/improved_25epochs_results.png (графики)")
    print(f"  - {filename} (отчет)")

if __name__ == "__main__":
    train_improved_25_epochs()
