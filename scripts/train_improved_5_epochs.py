#!/usr/bin/env python3
"""
Обучение Improved CNN на 5 эпохах
"""
...
def train_improved_5_epochs():
    """Обучение Improved CNN на 5 эпохах"""
    
    print("🚀 ОБУЧЕНИЕ IMPROVED CNN НА 5 ЭПОХАХ")
    print("=" * 60)
    ...
    # Результаты
    print(f"\n📈 РЕЗУЛЬТАТЫ IMPROVED CNN (5 ЭПОХ):")
    ...
    # Сохранение модели
    model_path = 'models/improved_5epochs_model.pth'
    torch.save({
    ...
    ax1.set_ylabel('Loss')
    ax1.set_title('Improved CNN (5 epochs): Loss по эпохам')
    ax1.legend()
    ...
    ax2.set_title('Improved CNN (5 epochs): Accuracy по эпохам')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('results/5_epochs/improved_5epochs_results.png', dpi=150, bbox_inches='tight')
    plt.show()
    
    print(f"✅ Графики сохранены в results/5_epochs/improved_5epochs_results.png")
    
    # Сохранение отчета
    print(f"\n📝 Создание отчета...")
    
    filename = 'results/5_epochs/improved_5epochs_report.txt'
    
    with open(filename, 'w', encoding='utf-8') as f:
        f.write("ОТЧЕТ ОБУЧЕНИЯ IMPROVED CNN НА 5 ЭПОХАХ\n")
        f.write("=" * 50 + "\n\n")
        
        f.write(f"Модель: Improved CNN (5 epochs)\n")
    ...
    print(f"  - results/improved_5epochs_results.png (графики)")
    print(f"  - {filename} (отчет)")

if __name__ == "__main__":
    train_improved_5_epochs()
